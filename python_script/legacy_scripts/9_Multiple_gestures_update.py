...
while True:
    ...
    if not latest_result or not latest_result.hand_landmarks:                                          # If there's no hand
        index_initialized = False
        swipe_start_pos = None # Reset swipe tracking if hand is lost
    ...
    if current_status_swipe != last_sent_status_swipe:
        ...
        if current_status_swipe != "no_gesture": # Only send actual gestures
            firebase_thread = threading.Thread(target=set_firebase_status, args=(current_status_swipe,))
