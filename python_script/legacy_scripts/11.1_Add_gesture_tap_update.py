...
def check_swipe(current_index, start_index):
    index_x, index_y = current_index
    start_index_x, start_index_y = start_index
    
    y_movement = index_y - start_index_y
    x_movement = index_x - start_index_x
    if (abs(y_movement) > abs(x_movement)):
        handymen.submit(set_firebase_gesture, "swipe_down" if y_movement > 0 else "swipe_up")
    else:
        handymen.submit(set_firebase_gesture, "swipe_right" if x_movement > 0 else "swipe_left")
# ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    if not Hand_landmarks_containing_object or not Hand_landmarks_containing_object.hand_landmarks:
        ...
        for hand_idx, hand in enumerate(Hand_landmarks_containing_object.hand_landmarks):
            ...
            if last_index_x is None:
                # If last values of the coords are None then that means this is the first frame with a hand.
            ...
            # Distinction between swipe and tap
            if v_wrist > 12:
                is_hand_stable = False
                index_start_position = None
            else:
                is_hand_stable = True
                if index_start_position is None:
                    index_start_position = (kalman_index_x, kalman_index_y)
            
            if is_hand_stable and index_start_position is not None:
                v_z = abs(v_z)
                if (v_xy > SWIPE_VELOCITY_THRESHOLD) and v_z < STILLNESS_THRESHOLD_Z:
                    check_swipe((kalman_index_x, kalman_index_y), index_start_position)
                elif (v_z > TAP_VELOCITY_THRESHOLD) and v_xy < STILLNESS_THRESHOLD_XY:
                    check_tap()
