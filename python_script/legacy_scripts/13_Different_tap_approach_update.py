...
while True:
    ...
    if not Hand_landmarks_containing_object or not Hand_landmarks_containing_object.hand_landmarks:
        ...
        for hand_idx, hand in enumerate(Hand_landmarks_containing_object.hand_landmarks):
            ...
            # Applying the kalman filter on middle and storing kalmanised coords in new variables
            ...
            # Delta calculation for swipes
            delta_x = index_x - last_frame_coordinates[0][0]
            delta_y = index_y - last_frame_coordinates[0][1]
            # ----------------------------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            #         reader.putText(frame, "MODE: Tap", (50, 15), reader.FONT_HERSHEY_COMPLEX_SMALL, 0.5, (0, 0, 0), 1)
            #         if abs((velocities[0][2] + velocities[1][2]) / 2) > TAP_VELOCITY_THRESHOLD:
            #             handymen.submit(set_firebase_gesture, "tap")
                    
            #     elif gesture_mode == "SWIPE_MODE":
            #         reader.putText(frame, "MODE: Swipe", (50, 25), reader.FONT_HERSHEY_COMPLEX_SMALL, 0.5, (0, 0, 0), 1)
            #         v_xy = (velocities[0][0]**2 + velocities[0][1]**2)**0.5
            #         if v_xy > SWIPE_VELOCITY_THRESHOLD:
            #             if (abs(delta_x) > abs(delta_y)):
            #                 handymen.submit(set_firebase_gesture, "swipe_left" if delta_x < 0 else "swipe_right")
            #             elif (abs(delta_x) < abs(delta_y)):
            #                 handymen.submit(set_firebase_gesture, "swipe_up" if delta_y < 0 else "swipe_down")
            #         pass
            #     else:
            #         reader.putText(frame, "MODE: None", (50, 35), reader.FONT_HERSHEY_COMPLEX_SMALL, 0.5, (0, 0, 0), 1)
            #         pass
            if velocities[2][0] > 3.7:
                reader.putText(frame, "Hand too fast.", (20, image_height - 20), reader.FONT_ITALIC, 0.5, (0, 0, 0), 1)
                continue
